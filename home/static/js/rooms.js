...
ws.onopen = () => {
    ...
    if (created == "created") {
        isCreated = true;
        console.log("Room created, starting media stream");
        startMediaStream();
        ws.send(JSON.stringify({
            command: 'join',
            'username': userName,
        ...
        isCreated = false;
        console.log("Joining an existing room, starting media stream");
        startMediaStream();
        ws.send(JSON.stringify({
            command: 'join',
            'username': userName,
    ...
    if (data.type === 'chat_message') {
        ...
        const userIconHTML = `
            <div name="userIcon" class="flex flex-wrap h-fit relative justify-center joinedUser" userusername="${data.username}">
                ...
                <audio id="our_audio" class="hidden"></audio>
            </div>
        `;
...
function createOffer() {
    ...
    // Add local stream tracks only once
    stream.getTracks().forEach((track) => {
        rtcpeerconnection.addTrack(track, stream);
    });
...
function onTrack(event) {
    ...
    setTimeout(() => {
        let remote_audio = document.createElement('audio');
        remote_audio.classList.add('hidden');
